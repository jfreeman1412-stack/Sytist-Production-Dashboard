...
**Status:** Phase 49 v2 — production. See `SPEC.md` for the full phase history and `CHANGELOG.md` for a chronological summary.
...
| 60    | Digital-only orders get a 4th `digital` workflow bucket — stops `$0.00`-shipping download orders from being misclassified as League by the cost fallback. `categorizeShipping(…, isDigitalOnly)` + matching `_buildWorkflowSqlPredicate` digital bucket / league-fallback carve-out (digital-only = no physical cart row, accounting for Phase 45 `cart_download=0` digital packages); same `isDigitalOnly` SQL in list/detail/counts for parity. New amber Digital tab + badge + Home stat card. Option-name match still always wins | ✅ |
| 60a   | Instant-pack eligibility classification (badge only; bulk actions are 60b). New per-SKU `instantPackEligible` flag on `packaging-config.json` (default-deny) + `isInstantPackEligible(sku)` + a Settings → Packaging checkbox column. Order is eligible iff ≥1 physical item AND every physical item's SKU is eligible (specialty/drop-ship are physical → disqualify via default-deny; digital/modifier add-ons ignored). Computed purely in JS over expanded line items in list + detail (no SQL predicate — display-only; 60b's tab/count will need SQL parity). ⚡ Instant-Ship badge on the orders table. Harness 11/11 | ✅ |
| 61    | Fail-closed processing — stop partial/wrong-product shipments (order 112054 shipped 1 of 2 items). `_processSubOrder` now hard-fails (`success:false`, early return) on any photo-download failure (regular AND specialty — reverses Phase 55 soft-fail) and on green-screen/composite/imposition throws (wrong-product); `allOk` short-circuits cleanup/SS/status/notes so nothing partial ships. `_downloadWithRetry` adds timeout + backoff + terminal-vs-transient + `err.cause` logging. Visibility: OrderDetailPage red banner, `JobProgressBanner` always-visible failure enumeration, orders-list ⚠ Last-process-failed badge (from process-history). Placeholder-logo/team-photo/asset-fetch cases deferred to Phase 62. Harnesses 6/6 + 6/6 | ✅ |

| 61a   | Digital-by-config false-block fix. A standalone digital package (5D, `cart_download=0`) reached `printableItems` and false-blocked its order at the Phase 61 gate (`no_photo_url`). `_splitIntoSubOrders` (now async) also excludes `isDigital(sku)` items + hoists one `getProductWeights()` read; `no_photo_url` push now records `sku`. Exclusion keys on digital-ness, NOT missing-photo — a real print whose photo fails still hard-fails (harness guard proves it). Gate unchanged. Harness 8/8 | ✅ |
