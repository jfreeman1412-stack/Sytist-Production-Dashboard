...
## Phase 74 — Rotation for photo + graphic slot kinds (composite layout designer)

- **Audit-confirmed gap:** the composite layout designer's Rotation FormRow was inside an `{isText && …}` block, so only text slots had a Rotation control — `playerPhoto`, `playerBackground`, `teamPhoto`, `logo`, and `staticGraphic`/overlay slots had no rotation, designer or render. Phase 22 added text rotation; the photo/graphic side was never built. Going symmetric in one pass (vs spot-fix on teamPhoto only) — the work is shared and Joey wants rotation on the others eventually.
- **Render semantic chosen:** stable bounding box, corners clip. A rotated photo keeps its w × h slot rect; pixels that rotate outside the rect are cropped at the slot edge (the opposite of Phase 22's "text rotation extends the SVG to the slot diagonal" — see CLAUDE.md landmine). Reason: a rotated photo spilling outside its box would overlap neighboring slots and break print alignment; text spilling is harmless because the text-slot rect was always notional.
- **Server:** new `compositeService._applyRotationToBox(buffer, w, h, angleDeg)`. Fast path `if (!angleDeg) return buffer;` — byte-identical for layouts with no `rotation` field. Else `sharp(buffer).rotate(angleDeg, { background: transparent })` (returns a larger bounding box), then `extract` the central w × h region (= "stable bounding box, corners clip"). Wired into the **five** photo branches between the fit step and `_clipToCanvas`: playerPhoto, playerBackground, teamPhoto, logo, staticGraphic. Text path `_textSvg` untouched (preserves Phase 22 semantic).
- **Client designer canvas (`LayoutCanvas.js`):** each of the three `<image>` elements (graphic URL, live logo, live photo) gets `transform={imageTransform}` (= `rotate(angle pivotX pivotY)` where pivot is the slot center) AND `clipPath={clipPathRef}`. A per-slot `<defs><clipPath id="slot-clip-{idx}" clipPathUnits="userSpaceOnUse"><rect …slot rect…/></clipPath></defs>` defines the clip rect inside the slot's `<g>`. SVG primitives mirror the server semantic exactly: the rotated `<image>` is clipped to the original (un-rotated) slot rect. `idx` prop threaded through from the parent loop for the clipPath ID. **Designer placeholder branch** (used when no live photo / uploaded graphic / backdrop — i.e. a fresh layout the operator is editing for the first time): the colored placeholder `<rect>` and its label `<text>` ALSO carry `transform` + `clipPath` so rotation is visible without live data. Without this, the Rotation field's button looked like a no-op in the designer's main use case.
- **Designer UI (`LayoutDesignerPage.js`):** Rotation FormRow hoisted out of `{isText && (…)}` block in `QuickEditPanel` — sits between the geometry (X/Y/W/H) block and the `{isImage && (Fit mode)}` block, with a multi-line comment explaining the universal slot property + text-vs-photo render-semantic difference. `numericFields` already included `rotation` — no schema change.
- **Bug caught by the verification harness — DON'T regress:** the first cut composited the rotated (larger) buffer onto a fresh w × h transparent canvas with negative top/left offsets. `sharp.composite` REFUSES inputs larger than the destination canvas ("Image to composite must have same dimensions or smaller") and threw silently upstream — every rotated render produced an entirely transparent slot (canvas bg painted through where the rotated content should be). The `extract` primitive is the right one for "stable bounding box, corners clip"; the harness's "30° center is fill color" assertion is what caught this.
- **Verification:** `server/scripts/verify-rotation.js` 20/20 — for each of the 5 photo kinds: rotation 0° (fast-path center is fill color), 30° (center stays fill + un-rotated corner reveals canvas bg = corners clip), 90° (render succeeds, center is fill). Uses solid-color test PNGs per kind so a center pixel uniquely identifies which slot rendered. JPEG-compression tolerance (±10 RGB) on color comparisons since the sheet buffer is JPEG. Pre-existing layout files (no `rotation` field) hit the helper's `!angleDeg` fast path → byte-identical to pre-Phase-74 output. Live designer + live render verification is the operator's (per the verification rule: harness is not enough on display-affecting changes).
- **Out of scope (intentional):** OverrideEditorPage rotation UI (a separate surface from the layout designer; layout-author choice is what Phase 74 covers — the editor stays "edit text content/color, accept layout author's geometry"). Phase 22 text path semantics. Sub-pixel anti-alias smoothing of rotated edges (sharp's default bilinear is acceptable for the photo sizes involved).
