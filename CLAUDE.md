...
### Non-product line items false-block the fail-closed gate (Phase 64 — recurring pattern)

**Non-product line item types — `booking`, `preSell`, `preRegister`, and any future variant — can carry no SKU and no photo, and will reach the printable set / fail-closed gate as a "printable item with no photo" and hard-fail the whole order unless they are explicitly skip-flagged.** We hit this twice in one week: digital-by-config (5D, Phase 61a) and pre-registration (order 112376, Phase 64). The order's *real* prints get held hostage by one non-product line.

**The fix pattern (always the same):**
1. **Identify the type via a dedicated Sytist column**, never by name or by "missing photo": `booking → ms_cart.cart_booking`, `preSell → cart_pre_sell`, `preRegister → cart_pre_register_id`. Before assuming a type is already covered, **audit the `cart_*` column set in `ms_cart` for sibling flags** — that's how you find the next one.
