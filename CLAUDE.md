...
- **Package weights are FLOORED to whole oz (min 1) before SS gets them** (Phase 58, universal across packages and carriers). USPS gives a 1oz grace per package, so 8.7oz labelled 8oz is safe and saves a rate tier. The floor lives in `packagingService._buildResult` (both normal + fallback paths) and is idempotently re-applied in `shipstationService` at the order-level weight resolution (so operator overrides + the no-engine `defaultWeightOz` fallback are also floored). The floor MUST happen at the **ounce** step before the `OZ_TO_G = 28.3495` grams conversion — grams-only clamps would let 0.5oz become 1g (~0.035oz) which the label rounds to 0. The packaging engine's "roll remainder onto first non-digital line item" mechanism (so `Σ items = order weight`, what SS bills) now handles BOTH signs — under floor the remainder is negative — and clamps the absorbing item ≥1oz. Log line is `Floor rounding: -X.X oz` (sign-carrying via the `roundingDeltaOz` field); the old `Ceiling rounding: +X oz` / `preCeilingOz` / `ceilingRemainderOz` labels were renamed for honesty.
