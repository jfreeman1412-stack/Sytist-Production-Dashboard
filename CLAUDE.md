...
**JS↔SQL parity scope (A2 + Option X — addon blocking-side closed at ship).** SQL handles: individual eligible SKUs + package parents (precomputed passing-list) + all 7 skip-flag columns + digital-by-config exclusion **+ Option X: blocking-side addon check** (precomputed `blockingAddonOptIds` from `_loadInstantPackBlockingAddonOptIds`, two NOT EXISTS clauses joining `ms_cart_options` × {`ms_cart`, `ms_cart_archive`}) **+ Phase 79: workflow gate at the outermost level.** The OVERCOUNT direction is closed for both addons (Option X) and workflows (Phase 79). The badge (JS) is canonical for per-row decisions; the filter (SQL) is the high-confidence subset.
