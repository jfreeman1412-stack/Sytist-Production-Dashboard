...
### Instant-pack eligibility — JS canonical for the badge, SQL parity for the filter, workflow-gated to ship_to_home (Phase 60a + 77 + 79)

`order.isInstantPackEligible` (the orders-list ⚡ Instant-Ship badge + the order-detail shape) is computed in JS by `sytistDbService._computeInstantPackEligibility(lineItems, predicates, workflow)`, called in **both** `getOrdersByWorkflow` and `getOrderById` over the already-expanded canonical line items. The SQL counterpart `sytistDbService._buildInstantPackSqlPredicate(eligibleSkuList, digitalSkuList, blockingAddonOptIds, workflowGate)` is wired into `getOrdersByWorkflow` behind the `instantShipOnly` arg. Both encodings are gated by the SAME rule (Phase 79); the harness `verify-instant-pack-eligible.js` asserts JS↔SQL parity on the gate.

**The rule (in order — fail-fast on the cheapest check):**

1. **(Phase 79) `workflow === 'ship_to_home'`.** Literal allow-list. Manager / league / digital workflows are categorically ineligible regardless of SKU mix — they're hand-delivery / download-only flows, never packed-and-shipped. Future workflow values default-deny until explicitly added; **do NOT use a deny-list** (`!== 'ship_to_managers' && !== 'ship_to_league'`) — a future `'ship_to_custom'` would silently pass under that shape.
2. **(Phase 60a)** ≥1 physical item exists. "Physical" = not isPackageHeader, none of `INSTANT_PACK_SKIP_FLAGS`, not digital-by-config (case-tolerant lookup).
3. **(Phase 60a)** Every physical item's SKU is on the eligible list (`packaging-config.json` `productWeights[sku].instantPackEligible === true`).

The order of clauses is meaningful: the workflow gate is the cheapest check (one string comparison) and catches the largest mistake-cost class — Phase 79's pre-fix audit found ~16% of currently-SQL-eligible orders were non-home (798 manager + 508 league out of 8,079) that Phase 60a/77 had been silently overcounting since their shipdates. The gate is enforced at the SQL side at the OUTERMOST level (AND-composed with the SKU predicate via `_buildWorkflowSqlPredicate('ship_to_home', …)`), so it applies regardless of what the URL `workflow` filter is (e.g. operator picks `workflow=all` + `instantShipOnly=true` → SQL still narrows to home).

**Critical: the workflow gate uses the SAME `_buildWorkflowSqlPredicate` helper as the URL workflow filter.** Single source of truth: a future change to ship_to_home classification (e.g. new shipping-option names added to `SHIPPING_MAP.ship_to_home` in `shipping-mapping.json`) flows through to both gates automatically. **Don't reimplement the ship_to_home classification inline** in `_buildInstantPackSqlPredicate` — that would create a drift surface the next time SHIPPING_MAP changes. (`_buildInstantPackSqlPredicate` now returns `{sql, params}` instead of bare `sql` so the workflow gate's bound option-name array survives into the outer query — the call site at `getOrdersByWorkflow` does `where.push(p.sql); params.push(...p.params)`. Matches `_buildWorkflowSqlPredicate`'s precedent shape.)

**JS↔SQL parity scope (A2 + Option X + Phase 80 — all overcount paths closed at ship).** SQL handles: individual eligible SKUs + package parents (precomputed passing-list) + all 7 skip-flag columns + digital-by-config exclusion **+ Option X: blocking-side addon check** (precomputed `blockingAddonOptIds` from `_loadInstantPackBlockingAddonOptIds`, two NOT EXISTS clauses joining `ms_cart_options` × {`ms_cart`, `ms_cart_archive`}) **+ Phase 79: workflow gate at the outermost level** **+ Phase 80: blocking-package-parent check** (precomputed `blockingPackageParentSkuList` from `_loadInstantPackBlockingPackageParentSkuList`, two NOT EXISTS clauses over `ms_cart` × `ms_cart_archive` that check `UPPER(cart_sku)` against the blocking list — **NO skip-flag conditions**, deliberate; see the load-bearing detail below). The OVERCOUNT direction is closed for addons (Option X), workflows (Phase 79), AND package-parents-hidden-by-download-flag (Phase 80). The badge (JS) is canonical for per-row decisions; the filter (SQL) is the high-confidence subset.

**Phase 80 load-bearing detail — the `cart_download` guard is DELIBERATELY absent from clause (E).** The bug root: Sytist propagates `cart_download=1` to Silver Package (SKU 2) and Gold/Super Star Package (SKU 1) parents because both contain SKU 25 (Digital Download) as a constituent. Phase 77's `_physicalForInstantPackSqlFragment` requires `cart_download=0` — CORRECTLY, for individual SKUs where `cart_download=1` means "this row is a download, skip." Package parents are a DIFFERENT concern: the parent row's flag says nothing about its constituents. So the Phase 80 clause (E) does a **bare** `UPPER(cart_sku) IN (blockingList)` check with **zero skip-flag conditions** — the whole point is to catch package parents Sytist has flagged as download. **Do NOT "fix" this by folding it into `_physicalForInstantPackSqlFragment`** — that would break clauses A/B for individual SKUs and re-open the bug from the other side. Additive, localised clause; the `cart_download` asymmetry is the fix, not a bug in the fix.

**If a future package is added that (a) includes any Digital Download-flagged constituent AND (b) includes any ineligible non-digital constituent, it inherits this bug pattern automatically.** `_loadInstantPackBlockingPackageParentSkuList()` reads packages fresh each query — the new package lands on the blocking list at next request, no restart needed. This is why the loader is dynamic; hardcoding `{1, 2}` would need manual maintenance. Same read-fresh-each-query contract as `_loadDigitalSkuList` / `_loadInstantPackEligibleSkuList`.

**Critical sizing lesson from Phase 77's first cut (recorded so a future audit doesn't repeat the error):** addon divergence sizes against the **in-filter population**, NOT the overall population. The audit's initial 3.1% figure (orders with any addon, all open orders since 2026-05-01) was the wrong denominator: addons cluster on exactly the eligible parent SKUs (Memory Mate buyers commonly +$8.49 for the 2-magnet addon, where Memory Mate is eligible and Magnets are default-deny ineligible). In live data the no-addon-handling first cut hit **50% overcount in the SQL-filtered set** (3 of 6 SQL-eligible orders had badge=false), making the filter operationally a UX trap. Option X went in same-phase to close the overcount. **When estimating divergence cost for any future similar JS↔SQL parity work, size against the population the filter will actually return — not the overall population.**

**Phase 79's pre-audit caught a sibling-class bug**: of currently-SQL-eligible orders, **~16% (1,306 of 8,079) were non-home** (798 manager + 508 league) that Phase 60a/77 had been silently overcounting since their shipdates. The workflow gate fixed this *and* future-proofs it (default-deny on unknown workflow values). Same lesson surface: a forensic check on the actual SQL-returned population, BEFORE assuming the rule's behavior matched operator intent, surfaced a real bug instead of pure future-proofing. The audit-first discipline keeps catching these.

**What's still deferred (the UNDERCOUNT direction):** an order whose only eligible item is a product-type addon (parent is skip-flagged AND has an eligible product addon). JS would see the parent skipped via skip flag AND the addon synthetic *also* skipped because addon flags inherit from parent via `...li.flags` spread — so this case is actually not a real divergence under the current addon-flag-inheritance model. Genuinely-undercount cases require the addon to NOT inherit the parent's skip flag, which doesn't happen in `_expandAddonLineItems`. So in practice the deferred undercount surface is empty under today's expansion semantics; the gap exists only as a theoretical guarantee for some future expansion change. **The badge (JS) stays canonical for per-row decisions; the filter (SQL) is the high-confidence subset.** They agree in the live data we tested.

**The maintenance contract that MUST hold** (this is the load-bearing piece — getting this wrong silently re-opens the bug class Phase 64/69 already fired twice for): the SQL skip-flag set in `_physicalForInstantPackSqlFragment` MUST stay in lockstep with the JS `INSTANT_PACK_SKIP_FLAGS` const. The current set is `download / giftCert / creditProduct / booking / preSell / preRegister / coupon` ⇄ `cart_download / cart_gift_certificate / cart_credit_product / cart_booking / cart_pre_sell / cart_pre_register_id / cart_coupon`. When a new non-product line type appears in Sytist's `ms_cart` with its own `cart_*` column (Phase 64 pre-registration, Phase 69 coupon — the pattern recurs), add it to **all of**: the JS const, the SQL fragment, the canonical line-item flag construction in `getOrderById` / `getOrdersByWorkflow`, AND every other skip-flag consumer (`processingService`, `darkroomService`, `packingSlipService`, `shipstationService`, `routes/shipstation.js` `_computeEligibility`) — same Phase 64 lockstep rule. The Phase 64-style audit of `ms_cart`'s `cart_*` columns for siblings now has the SQL fragment as one of the update sites.

**Option X added a second lockstep rule:** when the addon-mapping config schema changes or new mapping types are introduced, `_loadInstantPackBlockingAddonOptIds` must be updated to recognise the new type's blocking-or-not classification. Today the rule is "product-type mapping whose mapped SKU isn't in eligible-list AND isn't digital → blocking opt_id." If a future mapping type wants to behave like a "synthetic line item that the SQL must consider", it needs the same precompute treatment. Modifier-type mappings stay ignored by design (they don't become line items).

Skipping any of these silently re-opens the JS↔SQL drift the Phase 60a landmine warned about. The Phase 77 first cut already shipped that lesson the hard way: the no-addon-handling A2 cut produced 50% overcount in the live filtered set before Option X went in.

Rule (default-deny): eligible iff **≥1 physical item** AND **every** physical item's SKU is `instantPackEligible:true` in `packaging-config.json` `productWeights[sku]`. *Physical* = NOT `isPackageHeader` AND none of `INSTANT_PACK_SKIP_FLAGS` AND NOT `isDigital(sku)`. Two things not to "fix":
...
**Package parent inclusion in the passing list.** `_loadInstantPackEligibleSkuList()` includes a package parent SKU iff every non-digital constituent is individually eligible AND ≥1 non-digital constituent exists. A 100%-digital package (every constituent `category='digital'`) is NOT in the passing list — JS expansion would yield zero physical items so the order can't satisfy ≥1, and the SQL gets there by simply not including the parent. Don't try to fold "all-digital packages are eligible because there's nothing to print" into the SQL via a special case — the existing logic already gets the right answer.
...
### Team dividers are BATCH-level, written once, last, gated all-or-nothing (Phase 63)

A league/manager "team batch" is **many orders for one team**, and the operator wants **exactly one** team divider as the **last sheet** of the whole batch (the closer). So divider generation is **batch-level, not per-order**: it lives in `_runBatchJob`'s after-loop (same slot as QR sheets), **not** in `_processSubOrder` (the old per-sub-order Step 4 was removed — it produced one divider per order, the wrong scope). Do not move it back into the per-order path.

- **It prints because it's a standalone `.txt`, placed last.** The divider is a `_DIVIDER_<team>.txt` (built by `darkroomService.buildDividerTxt` — 4 headers + a single `Qty=1`/`Size=5x8`/`Filepath=<jpg>`, no slip/prints) written into the **same shared output folder** as the batch's order `.txt` files, **after** all of them. Darkroom prints in **placement order**, so last-placed = prints-last. The write-ordering is guaranteed because the after-loop runs only after every per-order `await processOrder` (hence every atomic `writeTxtFile` rename). A divider JPG on disk that isn't referenced by a `.txt` does **not** print (the original bug) — the `.txt` is what matters.
- **Strict all-success gate (`_shouldWriteBatchDivider`) is load-bearing — don't loosen it.** Write the divider **only if** every order in the batch succeeded (and not cancelled, team present, requested). If any order failed, write **no** divider and show the skip banner. Reason: placement-order = print-order, so a divider written over a partially-failed batch stops being the closer the moment the failed orders are reprocessed *after* it. Lenient "≥1 success" was explicitly rejected for this. (Same principle as the Phase 61 fail-closed gate; batch-level cousin of the multi-team all-or-nothing gap.)
- **Field-verification status (read before trusting the negative path):** the positive case (all-success → one divider written last, prints as the closer) was field-verified at ship. The **negative case (one failed order ⇒ NO `_DIVIDER_*.txt` + skip banner) is harness-verified only** (`verify-batch-divider.js`) — NOT yet confirmed on a live batch. The operator opted to field-verify it on the next **natural** failed-order divider batch rather than fake a failure. **First time a divider batch hits a real download/processing failure, confirm: no `_DIVIDER_*.txt` was written, and the banner showed "Team divider not written — N of M orders failed."** Until that's seen live, treat the negative path as proven-in-harness-only.
- **Team identity is passed from the client, not derived.** `OrdersListPage.startBatch` sends `team:{subGalleryId,subGalleryName}` from the active Team filter and blocks if dividers are requested with no team filtered. Single-team batches make this unambiguous; don't add derive-from-orders or productName heuristics.
- **Intentional side-effects (not regressions):** single-order `/process`'s `generateDivider` is a no-op (dividers are batch-only by design since Phase 12); OrderDetailPage's `sub.dividerPath` line stops rendering (`&&`-guarded).
- **Parked:** the shared-output-dir assumption holds at `folder-sort.json = no_sort` (one folder per batch). If folder-sort is changed to split single-team orders into per-player/per-order subfolders, "one divider folder" is ambiguous — revisit then.
...
3. **Wire the flag into every skip set that lists the existing siblings** — keep them in lockstep: `processingService` `SKIP_FLAGS` (the gate), `darkroomService`, `packingSlipService`, `shipstationService`, `INSTANT_PACK_SKIP_FLAGS` in `sytistDbService`, the inline eligibility check in `routes/shipstation.js`, **AND `_physicalForInstantPackSqlFragment` (Phase 77 — the SQL-side cousin of `INSTANT_PACK_SKIP_FLAGS`; the new column name must be added to the cart_* checks for both `c` and `ca` aliases or the JS↔SQL parity gap re-opens).**
