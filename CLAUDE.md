...
### Instant-pack eligibility — JS canonical for the badge, SQL parity for the filter, addon gap documented (Phase 60a + 77)

`order.isInstantPackEligible` (the orders-list ⚡ Instant-Ship badge + the order-detail shape) is computed **purely in JS** by `sytistDbService._computeInstantPackEligibility(lineItems, predicates)`, called in **both** `getOrdersByWorkflow` and `getOrderById` over the already-expanded canonical line items. **Phase 77 delivered the SQL parity** the Phase 60a landmine warned would be needed when a filter / count surface appeared — the orders-list "⚡ Instant-Ship Only" toggle is that filter. The SQL predicate is `sytistDbService._buildInstantPackSqlPredicate(eligibleSkuList, digitalSkuList)`, wired into `getOrdersByWorkflow` behind the `instantShipOnly` arg.

**JS↔SQL parity scope (A2 + Option X — addon blocking-side closed at ship).** SQL handles: individual eligible SKUs + package parents (precomputed passing-list) + all 7 skip-flag columns + digital-by-config exclusion **+ Option X: blocking-side addon check** (precomputed `blockingAddonOptIds` from `_loadInstantPackBlockingAddonOptIds`, two NOT EXISTS clauses joining `ms_cart_options` × {`ms_cart`, `ms_cart_archive`}). The OVERCOUNT direction is closed: an eligible parent + ineligible product addon is now correctly classified ineligible by both JS and SQL.
